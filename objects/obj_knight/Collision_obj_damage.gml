...
if (z > (other.z - other.height) && (z - height) < other.z) {
	...
	if (other.damage_id != invincible_to) { //If the enemy was not hurt by this instance
		...
		//Takes knockback
		var _kb_val = other.knockback*(1 - kb_resist);
		if (_kb_val > 0) {
		...
		instance_create_layer(x, y - 24, "Instances", obj_sparkle_particle);
		
		//Plays the hit sound
		audio_play_sound(choose(snd_iron_hit_1, snd_iron_hit_2), 10, false);
